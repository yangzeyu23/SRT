classdef coreFresnelLayer < nnet.layer.Layer %#codegen
    ...
    methods
        function layer = coreFresnelLayer(inputSize,outputSize,coreMask,P,NameValueArgs)
            ...
            layer.Description = "Fresnel proagation of core phase-only input";
           
            % Define nested layer graph.
            lgraph = layerGraph;
            layers = [
                imageInputLayer([inputSize 1],'Normalization','None','Name','in')
%                 vectorizingLayer('vectorize')
%                 corePhaseToSLMLayer(coreCenter,inputSize,weights,'coremap')
                plusLayer(P,'plus')];
            ...
            % add core mask
            lgraph = addLayers(lgraph,hadamardProdLayer(coreMask,'cosmask'));
            lgraph = addLayers(lgraph,hadamardProdLayer(coreMask,'sinmask'));
            lgraph = connectLayers(lgraph,'cos','cosmask');
            lgraph = connectLayers(lgraph,'sin','sinmask');
            ...
            lgraph = connectLayers(lgraph,'cosmask','cospad');
            lgraph = connectLayers(lgraph,'sinmask','sinpad');
